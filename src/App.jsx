...
function App() {
  ...
  useEffect(() => {
    const fetchAlbums = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/deezer-api/search/album?q=${encodeURIComponent(debouncedSearchTerm)}`);
  ...
  const handleAlbumClick = (album) => {
    ...
    setIsModalOpen(true);
    console.log("Abriendo modal para álbum:", album.title);
  };
  ...
  return (
    <div className="app-container">
      <header className="app-header">
        <h1 className="text-4xl font-bold text-center text-gray-800 mb-8">Reproductor Musical</h1>
